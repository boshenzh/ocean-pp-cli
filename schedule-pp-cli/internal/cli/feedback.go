...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".schedule-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SCHEDULE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SCHEDULE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/boshenzh/ocean-pp-cli/schedule-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.schedule-pp-cli/feedback.jsonl.
When ` + "`SCHEDULE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SCHEDULE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  schedule-pp-cli feedback list
  schedule-pp-cli feedback list --limit 5
  schedule-pp-cli feedback list --json`,
