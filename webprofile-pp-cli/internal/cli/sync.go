...
Exit codes & warnings:
  ...
  webprofile-pp-cli sync

  # Sync specific resources only
  webprofile-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  webprofile-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  webprofile-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  webprofile-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  webprofile-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/webprofile-pp-cli/data.db)")
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"reporters": "/files/v1/app/reference/Reporters.json",
	}
