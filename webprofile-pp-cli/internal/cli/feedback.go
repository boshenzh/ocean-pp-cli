...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".webprofile-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("WEBPROFILE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("WEBPROFILE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/boshenzh/ocean-pp-cli/webprofile-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.webprofile-pp-cli/feedback.jsonl.
When ` + "`WEBPROFILE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`WEBPROFILE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  webprofile-pp-cli feedback list
  webprofile-pp-cli feedback list --limit 5
  webprofile-pp-cli feedback list --json`,
