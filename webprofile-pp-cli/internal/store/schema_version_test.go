...
// TestMigrate_AddsColumnsOnUpgrade_Trade verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_Trade(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE trade (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
	...
	rows, err := s.DB().Query(`PRAGMA table_info(trade)`)
	if err != nil {
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from trade after migrate", want)
		}
	}
}
