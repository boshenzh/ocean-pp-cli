...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "freightindex-pp-cli",
		Short: `Pulls the weekly Shanghai Containerized Freight Index, persists snapshots locally, and renders lane-filtered digests for ocean-freight forwarders.`,
		Long: `Pulls the weekly Shanghai Containerized Freight Index, persists snapshots locally, and renders lane-filtered digests for ocean-freight forwarders.

Highlights (not in the official API docs):
  • digest   Render the latest weekly snapshot as JSON or Markdown; optional --lane <fragment> repeatable filter narrows to the lanes the user cares about. No defaults — works for any forwarder regardless of lane…
  ...
  • compare-with-rate-store   Joins the user's quote workbook (xlsx) against the latest SCFI per lane and tags every row High/Low/Par with the USD/TEU gap. v0.1 ships a stub; full implementation lands when the rate-store reader d…
...
Health check: run 'freightindex-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("freightindex-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'freightindex-pp-cli profile list')")
	...
	rootCmd.AddCommand(newScfiPromotedCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("freightindex-pp-cli %s\n", version)
		},
	}
}
