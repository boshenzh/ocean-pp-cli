...
Exit codes & warnings:
  ...
  freightindex-pp-cli sync

  # Sync specific resources only
  freightindex-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  freightindex-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  freightindex-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  freightindex-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  freightindex-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/freightindex-pp-cli/data.db)")
