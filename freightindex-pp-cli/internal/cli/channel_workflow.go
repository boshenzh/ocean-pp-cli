...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  freightindex-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  freightindex-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  freightindex-pp-cli workflow status

  # Show status as JSON
  freightindex-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("freightindex-pp-cli")
			}
