...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".freightindex-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FREIGHTINDEX_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FREIGHTINDEX_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/boshenzh/ocean-pp-cli/freightindex-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.freightindex-pp-cli/feedback.jsonl.
When ` + "`FREIGHTINDEX_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FREIGHTINDEX_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  freightindex-pp-cli feedback list
  freightindex-pp-cli feedback list --limit 5
  freightindex-pp-cli feedback list --json`,
